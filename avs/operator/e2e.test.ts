...
async function loadDeployments(): Promise<Record<string, any>> {
  ...
  const warpAdsFilePath = path.join(
    __dirname,
    "..",
    "contracts",
    "deployments",
  ...
  const [coreDeployment, warpAdsDeployment] = await Promise.all([
    loadJsonFile(coreFilePath),
    loadJsonFile(warpAdsFilePath),
  ]);

  if (!coreDeployment || !warpAdsDeployment) {
...
describe("Operator Functionality", () => {
  ...
  let warpAdsServiceManager: ethers.Contract;
  ...
  beforeAll(async () => {
    ...
    const warpAdsServiceManagerABI = await loadJsonFile(
      path.join(__dirname, "..", "abis", "WarpAdsServiceManager.json")
    );
    ...
    warpAdsServiceManager = new ethers.Contract(
      deployment.warpAds.addresses.warpAdsServiceManager,
      warpAdsServiceManagerABI,
      signer
    );
    ecdsaRegistryContract = new ethers.Contract(
      deployment.warpAds.addresses.stakeRegistry,
      ecdsaRegistryABI,
      signer
    );
  ...
  it("should create a new task", async () => {
    ...
    const tx = await warpAdsServiceManager.createNewTask(taskName);
    await tx.wait();
  });
