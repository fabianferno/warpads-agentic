...
contract SetupPayments is Script, Test {
    ...
    function setUp() public {
        ...
        coreConfig = CoreDeploymentLib.readDeploymentConfigValues(
            "config/core/",
            block.chainid
        );
        warpAdsDeployment = WarpAdsDeploymentLib.readDeploymentJson(
            "deployments/warpads/",
            block.chainid
        );
    ...
    function run() external {
        vm.startBroadcast(warpAdsConfig.rewardsInitiatorKey);

        if (rewardsCoordinator.currRewardsCalculationEndTimestamp() == 0) {
            startTimestamp =
                uint32(block.timestamp) -
                (uint32(block.timestamp) % CALCULATION_INTERVAL_SECONDS);
        } else {
            startTimestamp =
        ...
        if (endTimestamp > block.timestamp) {
            revert(
                "End timestamp must be in the future.  Please wait to generate new payments."
            );
        }
    ...
    function createAVSRewardsSubmissions(
        ...
        ERC20Mock(warpAdsDeployment.token).mint(
            warpAdsConfig.rewardsInitiator,
            amountPerPayment * numPayments
        );
        ERC20Mock(warpAdsDeployment.token).increaseAllowance(
