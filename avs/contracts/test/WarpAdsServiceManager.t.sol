...
import {WarpAdsDeploymentLib} from "../script/utils/WarpAdsDeploymentLib.sol";
...
import {IWarpAdsServiceManager} from "../src/IWarpAdsServiceManager.sol";
import {ECDSAUpgradeable} from "@openzeppelin-upgrades/contracts/utils/cryptography/ECDSAUpgradeable.sol";

contract WarpAdsTaskManagerSetup is Test {
    ...
    WarpAdsDeploymentLib.DeploymentData internal warpAdsDeployment;
    ...
    function setUp() public virtual {
        ...
        labelContracts(coreDeployment, warpAdsDeployment);
    }
    ...
    function labelContracts(
        ...
        WarpAdsDeploymentLib.DeploymentData memory warpAdsDeployment
    ) internal {
        ...
        vm.label(
            warpAdsDeployment.warpAdsServiceManager,
            "WarpAdsServiceManager"
        );
        vm.label(warpAdsDeployment.stakeRegistry, "StakeRegistry");
    }
    ...
    function registerOperatorToAVS(Operator memory operator) internal {
        ECDSAStakeRegistry stakeRegistry = ECDSAStakeRegistry(
            warpAdsDeployment.stakeRegistry
        );
        ...
        bytes32 operatorRegistrationDigestHash = avsDirectory
            .calculateOperatorAVSRegistrationDigestHash(
                operator.key.addr,
                address(warpAdsDeployment.warpAdsServiceManager),
                salt,
                expiry
            );
    ...
    function createTask(
        ...
        IWarpAdsServiceManager warpAdsServiceManager = IWarpAdsServiceManager(
            warpAdsDeployment.warpAdsServiceManager
        );

        vm.prank(generator.key.addr);
        warpAdsServiceManager.createNewTask(taskName);
    }

    function respondToTask(
        Operator memory operator,
        IWarpAdsServiceManager.Task memory task,
        ...
        IWarpAdsServiceManager(warpAdsDeployment.warpAdsServiceManager)
            .respondToTask(task, referenceTaskIndex, signedTask);
    }
}

contract WarpAdsServiceManagerInitialization is WarpAdsTaskManagerSetup {
    function testInitialization() public view {
        ...
        assertTrue(
            ...
            "WarpAdsServiceManager not deployed"
        );
        assertTrue(
...
contract RegisterOperator is WarpAdsTaskManagerSetup {
    ...
    IWarpAdsServiceManager internal sm;
    ...
    function setUp() public virtual override {
        ...
        sm = IWarpAdsServiceManager(warpAdsDeployment.warpAdsServiceManager);
        stakeRegistry = ECDSAStakeRegistry(warpAdsDeployment.stakeRegistry);
...
contract CreateTask is WarpAdsTaskManagerSetup {
    IWarpAdsServiceManager internal sm;
    ...
    function testCreateTask() public {
        ...
        vm.prank(generator.key.addr);
        IWarpAdsServiceManager.Task memory newTask = sm.createNewTask(taskName);
...
contract RespondToTask is WarpAdsTaskManagerSetup {
    ...
    function testRespondToTask() public {
        ...
        bytes32 messageHash = keccak256(abi.encodePacked(taskName));
